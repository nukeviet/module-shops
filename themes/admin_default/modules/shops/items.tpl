    ...
    <form action="{NV_BASE_ADMINURL}index.php" method="get">
        ...
        <div class="row">
            ...
            <div class="col-xs-12 col-md-3">
                <div class="form-group">
                    <div class="input-group">
                        <input type="text" class="form-control" name="from" id="from" value="{FROM}" readonly="readonly" placeholder="{LANG.date_from}"> <span class="input-group-btn">
            ...
            <div class="col-xs-12 col-md-3">
                <div class="form-group">
                    <div class="input-group">
                        <input type="text" class="form-control" name="to" id="to" value="{TO}" readonly="readonly" placeholder="{LANG.date_to}"> <span class="input-group-btn">
        ...
        <input type="hidden" name="checkss" value="{CHECKSESS}" /> <em class="help-block">{SEARCH_NOTE}</em>
    </form>
</div>
        ...
        <table class="table table-striped table-bordered table-hover">
            <thead>
                <tr>
                    ...
                    <!-- END: asc -->
                        <!-- END: order_num_sell -->
                        <a href="{BASE_URL_NUM_SELL}">{LANG.num_selled}</a>
                    </th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="top">
                        <p>
                            <!-- BEGIN: sort --> <a href="javascript:void(0);" title="{LANG.order_by_number}: {ROW.weight}" onclick="nv_sort_content({ROW.id}, {ROW.weight})"><i class="fa fa-fw fa-sort" aria-hidden="true"></i></a> <!-- END: sort -->
                            <a target="_blank" href="{ROW.link}">{ROW.title}</a>
                        </p>
                        <div class="product-info">
                            {LANG.order_update}: <span class="other">{ROW.edittime}</span> | {LANG.content_admin}: <span class="other">{ROW.admin_id}</span>
                        </div>
                    </td>
                    ...
                    <!-- END: seller -->
                        <!-- BEGIN: seller_empty --> {ROW.num_sell} {ROW.product_unit} <!-- END: seller_empty -->
                    </td>
                    <td class="text-center">{ROW.status}</td>
                    <td class="text-center">
                        <!-- BEGIN: warehouse_icon --> <a href="{ROW.link_warehouse}" title="{LANG.warehouse}"><em class="fa fa-cubes fa-lg"> <!-- END: warehouse_icon --> &nbsp;
...
<div id="order_by" title="{LANG.order_by}">
    <strong id="order_by_title">{LANG.order_by}</strong>
    <form method="post" class="form-horizontal">
        <input type="hidden" name="{NV_NAME_VARIABLE}" value="{MODULE_NAME}" />
        <input type="hidden" name="{NV_OP_VARIABLE}" value="{OP}" />
        <input type="hidden" name="order_by" value="1" />
        <input type="hidden" name="order_by_id" value="0" id="order_by_id"/>
        <div class="form-group">
            <label for="order_by_number" class="col-sm-12 control-label">{LANG.order_by_number}</label>
            <div class="col-sm-12">
               <input type="number" class="form-control text-center w100" id="order_by_number"  value="" readonly="readonly">
            </div>
        </div>
        <div class="form-group">
            <label for="order_by_new" class="col-sm-12 control-label">{LANG.order_by_new}</label>
            <div class="col-sm-12">
               <input type="number" class="form-control text-center w100" name="order_by_new" id="order_by_new"  value="" min="1">
            </div>
        </div>
