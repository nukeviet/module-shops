...
{
    global $module_name, $global_array_group, $module_upload;
    ...
    if (!empty($subgroupid_i)) {
        foreach ($subgroupid_i as $groupid_i) {
            ...
            $image = '';
            if (!empty($data_group['image']) and file_exists(NV_ROOTDIR . '/' . NV_UPLOADS_DIR . '/' . $module_upload . '/' . $data_group['image'])) {
                $image = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload . '/' . $data_group['image'];
                $image = '<img src="' . $image . '" style="margin-top: -3px; max-width: 16px; max-height: 16px" alt="' . $data_group['title'] . '" />';
            }
