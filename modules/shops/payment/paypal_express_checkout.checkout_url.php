...
// Đường dẫn trả về
$returnUrl = NV_MY_DOMAIN . NV_STATIC_URL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op . '&order_id=' . $order_id . '&payment=' . $payment . '&checksum=' . md5($order_id . $payment . $global_config['sitekey'] . session_id()) . '&getexpresscheckoutdetails=1';
// Đường dẫn hủy thanh toán
$cancelUrl = NV_MY_DOMAIN . NV_STATIC_URL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op . '&order_id=' . $order_id . '&checkss=' . md5($order_id . $global_config['sitekey'] . session_id());
...
if ($nv_Request->isset_request("getexpresscheckoutdetails", "get")) {
    ...
    if (isset($getECResponse)) {
        if ($getECResponse->Ack == 'Success') {
            ...
            $doExpressURL = NV_MY_DOMAIN . NV_STATIC_URL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=complete&payment=" . $payment . "&paycomplete&token=" . $token . "&payerid=" . $payerID;
