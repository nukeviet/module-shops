...
if ($id > 0) {
    ...
    if ($checkss == md5($id . session_id() . $global_config['sitekey']) and $allowed_send == 1) {
        $link = $global_config['site_url'] . NV_STATIC_URL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $global_array_shops_cat[$catid]['alias'] . '/' . $alias . $global_config['rewrite_exturl'];
    ...
    if ($id > 0 && $allowed_send == 1) {
        ...
        $sendmail = array(
            ...
            'v_mail' => $youremail,
            'result' => $result,
            'action' => NV_STATIC_URL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=sendmail' //
        );
